...
void MainGame::initSystems()
{
	...
	ship.transformPositions(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y * deltaTime, ship.getTM().GetPos()->z), glm::vec3(ship.getTM().GetRot()->x - 90, ship.getTM().GetRot()->y, ship.getTM().GetRot()->z), glm::vec3(ship.getTM().GetScale()->x * 0.05, ship.getTM().GetScale()->y * 0.05, ship.getTM().GetScale()->z * 0.05));
...
void MainGame::processInput()
{
	...
	while (SDL_PollEvent(&evnt)) //get and process events
	{
		...
		case SDL_KEYDOWN:
			...
			{
			case SDLK_w:
				ship.transformPositions(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y + 20.0f * deltaTime, ship.getTM().GetPos()->z), glm::vec3(ship.getTM().GetRot()->x, ship.getTM().GetRot()->y, ship.getTM().GetRot()->z), glm::vec3(ship.getTM().GetScale()->x, ship.getTM().GetScale()->y, ship.getTM().GetScale()->z));
				myCamera.MoveUp(20.0f * deltaTime);
				break;
			case SDLK_a:
				ship.transformPositions(glm::vec3(ship.getTM().GetPos()->x + 20.0f * deltaTime, ship.getTM().GetPos()->y, ship.getTM().GetPos()->z), glm::vec3(ship.getTM().GetRot()->x, ship.getTM().GetRot()->y, ship.getTM().GetRot()->z), glm::vec3(ship.getTM().GetScale()->x, ship.getTM().GetScale()->y, ship.getTM().GetScale()->z));
				myCamera.MoveRight(20.0f * deltaTime);
				if (thirdPerson == true)
				{
					myCamera.setPos(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y - 10.0f, ship.getTM().GetPos()->z - 5.0f));
				}
				if (firstPerson == true)
				{
					myCamera.setPos(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y + 3.0f, ship.getTM().GetPos()->z - 5.0f));
				}
				break;
			case SDLK_s:
				ship.transformPositions(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y - 20.0f * deltaTime, ship.getTM().GetPos()->z), glm::vec3(ship.getTM().GetRot()->x, ship.getTM().GetRot()->y, ship.getTM().GetRot()->z), glm::vec3(ship.getTM().GetScale()->x, ship.getTM().GetScale()->y, ship.getTM().GetScale()->z));
			...
			//Top Down View
			case SDLK_c:
				myCamera.setPos(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y, ship.getTM().GetPos()->z - 50.0f));
				myCamera.setLook(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y, ship.getTM().GetPos()->z));
				thirdPerson = false;
				firstPerson = false;
				break;
...
void MainGame::drawMissiles()
{
	...
	for (int i = 0; i < 20; ++i)
	{
		...
		{
			missiles[i].transformPositions(glm::vec3(missiles[i].getTM().GetPos()->x, missiles[i].getTM().GetPos()->y + 10.0f * deltaTime, missiles[i].getTM().GetPos()->z), glm::vec3(missiles[i].getTM().GetRot()->x, missiles[i].getTM().GetRot()->y, missiles[i].getTM().GetRot()->z), glm::vec3(missiles[i].getTM().GetScale()->x, missiles[i].getTM().GetScale()->y, missiles[i].getTM().GetScale()->z));
...
{
	missiles[i].transformPositions(glm::vec3(ship.getTM().GetPos()->x, ship.getTM().GetPos()->y + 10.0f * deltaTime, ship.getTM().GetPos()->z), glm::vec3(ship.getTM().GetRot()->x, ship.getTM().GetRot()->y, ship.getTM().GetRot()->z), glm::vec3(ship.getTM().GetScale()->x * 2, ship.getTM().GetScale()->y * 2, ship.getTM().GetScale()->z * 2));
